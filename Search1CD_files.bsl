	...
	Для каждого ИмяДиска Из мМассивДисковДляПоиска Цикл
		...
		Для каждого КаталогИзМассива Из МассивКорневыхКаталоговДиска Цикл
			...
			Для каждого ФайлИзМассива Из МассивФайлов Цикл
				...
				Сообщить(ФайлИзМассива.ПолноеИмя);
			КонецЦикла;
		КонецЦикла;
		
		ПутьФайл = ОбъединитьПути(КаталогВременныхФайлов(), СтрШаблон("1C_search_%1.csv", Лев(ИмяДиска, 1)));
		
		ТекстДокумент = Новый ЗаписьТекста;
		ТекстДокумент.Открыть(ПутьФайл);
		инд = 0;
		Для каждого Элем Из мМассивНайденныеФайлы Цикл
			Инд = Инд + 1;
			ТекстДокумент.ЗаписатьСтроку(СтрШаблон("%1;%2;%3", Инд, Элем.ПолноеИмя, Элем.ПолучитьВремяИзменения()));
		КонецЦикла;
		ТекстДокумент.Закрыть();
